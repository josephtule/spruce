...
impl<'a> DynamicalSystem<'a> {
    pub fn rk4(&mut self, sat_num: usize) {
        ...
        state[0..3].copy_from_slice(&self.satellite[sat_num].position);
        state[3..6].copy_from_slice(&self.satellite[sat_num].velocity);
        // println!("The current state is {:.4?}",&state);
        let k1 = self.satellite[sat_num].dxdt(&state,&self.time);
        ...
        for i in 0..3 {
            self.satellite[sat_num].position[i] += self.step_width/6. * (k1[i] + 2.*k2[i] + 2.*k3[i] + k4[i]);
            self.satellite[sat_num].velocity[i] += self.step_width/6. * (k1[i+3] + 2.*k2[i+3] + 2.*k3[i+3] + k4[i+3]);
        }
